...
export type Expense = {
  ...
  title: string;
  amount: number;      // cents
  // pode vir string (YYYY-MM-DD), Date ou Timestamp; usaremos monthOf
  date: any;
  category: string;
  paidBy: "A" | "B";
...
export type Income = {
  ...
  source: string;
  amount: number;      // cents
  // compat: aceitamos qualquer uma dessas 3 formas
  ym?: string;         // NOVO: "YYYY-MM"
  month?: string;      // legado
...
// ✅ Usa o parâmetro ym e aplica monthOf em despesas e rendas
export function sliceByMonth(expenses: Expense[], incomes: Income[], ym: string): MonthSlice {
  return {
    ym,
    expenses: expenses.filter(e => !e.deleted && monthOf(e) === ym),
    incomes:  incomes.filter(i => monthOf(i) === ym),
  };
}
...
export function analyzeMonth(
  ...
  const drift = avg3 ? (totalOut - avg3) / avg3 : 0;
